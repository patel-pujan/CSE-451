...
struct segdesc {
  ...
  uint type : 4;       // Segment type (see STS_ constants, we're grouping E, DC, RW, and A flags of access byte).
  uint s : 1;          // 0 = system segment, 1 = code/data segment
  uint dpl : 2;        // Descriptor Privilege Level
  uint p : 1;          // Present
  uint lim_19_16 : 4;  // High bits of segment limit
  uint rsv : 1;        // Reserved
  uint l : 1;          // Long-mode code flag (1 for 64-bit code segments, in which case db should be 0)
  uint db : 1;         // 0 = 16-bit segment, 1 = 32-bit segment
  uint g : 1;          // Granularity: limit scaled by 4K when set
  uint base_31_24 : 8; // High bits of segment base address
};

#define SEG64(type, base, lim, l, dpl)                                         \
  (struct segdesc) {                                                           \
    ((lim) >> 12) & 0xffff, (uint64_t)(base)&0xffff,                           \
        ((uint64_t)(base) >> 16) & 0xff, type, 1, dpl, 1,                      \
        (uint64_t)(lim) >> 28, 0, l, 0, 0, (uint64_t)(base) >> 24              \
  }
