...
QEMUIO	:= -serial mon:stdio -nographic 
...
qemu-record: $(PROJECT)-qemu-record

qemu-replay: $(PROJECT)-qemu-replay

qemu-gdb-replay: $(PROJECT)-qemu-gdb-replay
...
turnin:
	$(ZIP) $(ZIPOPTS) $(TURNINNAME) inc kernel user Makefile mkfs.c sign.pl *.txt *.pdf